// TikTok Live Captions - by TikTools — OBS plugin entry point.
//
...
OBS_DECLARE_MODULE()
OBS_MODULE_USE_DEFAULT_LOCALE("obs-tiktok-captions", "en-US")

MODULE_EXPORT const char *obs_module_description(void)
{
    return "Real-time captions for TikTok Live by TikTools";
}

MODULE_EXPORT const char *obs_module_name(void)
{
    return "TikTok Live Captions - by TikTools";
}

static tiktok::TikToolCaptionsDock *g_dock = nullptr;
...
static void on_frontend_loaded(enum obs_frontend_event ev, void *)
{
    ...
    if (g_dock)
        return;
    g_dock = tiktok::TikToolCaptionsDock::install();
}

bool obs_module_load(void)
{
    blog(LOG_INFO, "[tiktok-captions] loading v%s", TIKTOK_CAPTIONS_VERSION_STR);
    tiktok::Settings::instance().load();
...
void obs_module_unload(void)
{
    ...
    tiktok::Settings::instance().save();
    blog(LOG_INFO, "[tiktok-captions] unloaded");
}
