...
void WsClient::connectWith(const QString &jwt, const QString &deviceId,
    ...
    QUrl url(QString::fromLatin1(TIKTOOL_WS_BASE) + "/captions/ws");
    QUrlQuery q;
    if (!jwt.isEmpty())            q.addQueryItem("jwtKey", jwt);
    if (!deviceId.isEmpty())       q.addQueryItem("deviceId", deviceId);
    if (!mode.isEmpty())           q.addQueryItem("mode", mode);
    ...
    q.addQueryItem("version", TIKTOOL_CAPTIONS_VERSION_STR);
    url.setQuery(q);
...
bool WsClient::parseHttpResponse() {
    ...
    blog(LOG_INFO, "[tiktool-captions] WS handshake complete");
    drainBuffer();
    return true;
}
...
void WsClient::onDisconnected() {
    bool wasOpen = state_ == State::Open;
    teardown();
    if (wasOpen) blog(LOG_INFO, "[tiktool-captions] WS disconnected");
    if (want_) scheduleReconnect();
}
...
void WsClient::onReconnectTick() {
    if (!want_) return;
    connectWith(jwt_, deviceId_, mode_, srcLang_, translateTo_);
}
