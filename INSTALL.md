...
Grab the matching archive from the latest release at https://github.com/tiktooldev/obs-tiktool-captions/releases.

### Windows

1. Close OBS.
2. Extract the zip.
3. Copy `obs-tiktool-captions.dll` into `C:\Program Files\obs-studio\obs-plugins\64bit\`.
4. Copy the `data\obs-plugins\obs-tiktool-captions\` folder into `C:\Program Files\obs-studio\data\obs-plugins\`.
5. Start OBS. Open `View -> Docks -> TikTool Live Captions`.

### macOS

1. Close OBS.
2. Unzip the archive.
3. Drag `obs-tiktool-captions.plugin` into `~/Library/Application Support/obs-studio/plugins/`.
4. Start OBS. Open `View -> Docks -> TikTool Live Captions`.
...
1. Close OBS.
2. Extract the tar.
3. Copy the tree into `~/.config/obs-studio/plugins/obs-tiktool-captions/` so you end up with:
   ```
   ~/.config/obs-studio/plugins/obs-tiktool-captions/bin/64bit/obs-tiktool-captions.so
   ~/.config/obs-studio/plugins/obs-tiktool-captions/data/locale/en-US.ini
   ~/.config/obs-studio/plugins/obs-tiktool-captions/data/style.qss
   ```
4. Start OBS. Open `View -> Docks -> TikTool Live Captions`.

## First run

When the dock appears the wizard opens automatically. Five short steps. Pick your mic, your language, hit Install Overlay, then Start Captions. The overlay browser source named `TikTool Captions` lands inside your active scene as a 1080x1920 vertical layer. Move and resize freely - the plugin remembers it.
